...
public class RestaurantController {
    ...
    public ApiResponse<Object> searchRestaurantV3Get(
            @RequestParam("latitude") Double latitude,
            @RequestParam("longitude") Double longitude
    ) {
        return ApiResponse.success(restaurantService.searchRestaurantV3Get(latitude, longitude));
    }

    @PostMapping("/search/v3")
    public ApiResponse<Object> searchRestaurantV3Post(
            @RequestParam("latitude") Double latitude,
            @RequestParam("longitude") Double longitude,
            @RequestBody RestaurantRequestDto restaurantRequestDto
            ) {
        return ApiResponse.success(restaurantService.searchRestaurantV3Post(latitude, longitude, restaurantRequestDto));
    }
